...
path_output_dir="IMGbugs/"


# cap = cv2.VideoCapture(0)
capLento = cv2.VideoCapture(0)
# cap = cv2.VideoCapture("rtsp://admin:Inviolavel1@177.101.141.181:555/cam/realmonitor?channel=0&subtype=0")
#capLento = cv2.VideoCapture("rtsp://admin:Inviolavel1@177.101.141.181:555/cam/realmonitor?channel=3&subtype=0")

capLento.set(CAP_PROP_FPS, 30)
...
while True:
    ...
    for face_loc, name in zip(face_locations, face_names):
        ...
        cv2.putText(frame,namePessoa, (x1,y1 -10), cv2.FONT_HERSHEY_COMPLEX, 1, (0,0,200), 3)
        ...
        if name != 'Unknown':
            #sendUDP.MandarUDP()
            if cont > 50:
                cv2.imwrite(os.path.join(path_output_dir, namePessoa + '_' + data+ '.png'), frame)
                cont = 0
            #tts.say("Olá" + namePessoa + "!")
            #tts.runAndWait
            #time.sleep(5)
